...
import {randomData} from '../line-chart/line-chart.component.sandbox';
...
let widget = {
  ...
  instances: ['en,sipuri,Credit Card',
...
const realTimeData = [
  {dataType: 'Queue Performance', instance: 'en,sipuri', measureName: 'SunBurstAnswered', measureTimestamp: 1570492800000, measureValue: 173},
  {instance: 'en,Mobile Video', measureName: 'SunBurstAnswered', measureTimestamp: 1570492800000, measureValue: 148, dataType: 'Queue Performance'},
  {instance: 'en,Mobile Video1', measureName: 'SunBurstAnswered', measureTimestamp: 1570492800000, measureValue: 148, dataType: 'Queue Performance'}
];
let processor = new RealTimeDataProcessorImpl(new SandBoxFormulaMeasureFactory());
let converter: ChartDataConverterService = new HighchartsDataConverterFactory(processor, new TimeUtilsImpl()).createSunburstConverter(
  widget, {});
let realData = converter.convert(realTimeData);
console.log(widget, realData);
appConfig.performanceLogging = true;

export default sandboxOf(SunburstComponent, {
  imports: [
  ...
  .add('Sunburst chart', {
    template: `<app-sunburst [widget]="widget" [data]="data"></app-sunburst>`,
    context: {
  ...
  .add('Sunburst chart for performance testing', {
    template: `<app-sunburst [widget]="widget" [data]="data"></app-sunburst>
<table>
      <tr>
        <td>Fake instance</td>
      ...
      onReload() {
        ...
        while (true) {
          ...
          for (let i = 0; i < maxLevel; i++) {
            fakeInstances.push(`${s},l_${i}`);
            if (fakeInstances.length > instanceFake) {
              break;
            }
          }
          if (fakeInstances.length > instanceFake) {
        ...
        for (let x = 0; x < instanceFake; x++) {
          for (let y = 0; y < measureFake; y++) {
            ...
            for (let i = 0; i < dataPoint; i++) {
              const measureValue = Math.floor(Math.random() * Math.floor(150));
        ...
        processor = new RealTimeDataProcessorImpl(new SandBoxFormulaMeasureFactory());
        converter = new HighchartsDataConverterFactory(processor, new TimeUtilsImpl()).createSunburstConverter(
          widget, {});
        realData = converter.convert(fakeData);
